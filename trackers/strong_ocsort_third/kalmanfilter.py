...
class KalmanFilterNew(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x, dim_z, dim_u=0, P=None, Q=None, R=None,x=None):
        if dim_x < 1:
        ...
        # self.P = None  # uncertainty covariance
        ...
        # self.R = np.diag([1, 1, 0.1, 10])  # measurement uncertainty
    ...
    def apply_affine_correction(self, m, t):
        ...
        # 状态向量变换
        self.x[:2] = m @ self.x[:2] + t          # 位置 [x, y] -> [0, 1]
        self.x[4:6] = m @ self.x[4:6]            # 速度 [vx, vy] -> [4, 5]
        self.x[2] *= scale                       # 高度 [h] -> [2]
        self.x[6] *= scale                       # 高度变化率 [vh] -> [6]

        # 协方差矩阵变换
        self.P[:2, :2] = m @ self.P[:2, :2] @ m.T    # 位置协方差
        self.P[4:6, 4:6] = m @ self.P[4:6, 4:6] @ m.T # 速度协方差
        self.P[2, 2] *= scale * scale                 # 高度协方差
        self.P[6, 6] *= scale * scale                 # 高度变化率协方差
        ...
        if not self.observed and self.attr_saved is not None:
            ...
            self.attr_saved["P"][2, 2] *= scale * scale
            self.attr_saved["P"][6, 6] *= scale * scale

            self.attr_saved["last_measurement"][:2] = m @ self.attr_saved["last_measurement"][:2] + t
            self.attr_saved["last_measurement"][2] *= scale  # 假设第3个元素是高度
