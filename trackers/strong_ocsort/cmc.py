...
class CMCComputer:
    def __init__(self, minimum_features=10, method="file"):
        assert method in ["file", "sparse", "sift", "ecc"]
        ...
        if method == "sparse":
            ...
            self.comp_function = self.applyEcc_v2
    ...
    def applyEcc_v2(self, frame, mask, tag):
        ...
        if downscale > 1.0:
            ...
            frame = cv2.resize(frame, (max(1, int(width // downscale)), max(1, int(height // downscale))))
        ...
        for i in reversed(range(maxLevel)):
            # print(i)
            H_level = H.copy()
            ...
            (cc, H_level) = cv2.findTransformECC(frame, preframe, H_level, warp_mode, criteria, None, 1)
